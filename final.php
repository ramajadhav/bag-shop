...
else
{
   ...
   foreach($injector as $n)
    {
        $occurence1=$occurence1+substr_count($cardnumber,$n);
    }
    echo $occurence;
    echo $occurence1;

 //procced for injecction check
    if($occurence>0 || $occurence1>0)
    {
    ...
    if($qry)
    {
        ...
        echo '<script>alert("SQL INJECTION is detected for 3 times !!!! your account has blocked");location.href="logout.php";</script>';
 ...
 $pmethod = $_GET['pmethod'];
    ...
    else
    {
        if(isset($_POST['payment']))
        {
            ...
            $r_date = date_add($issue_date,date_interval_create_from_date_string("2 days"));
